...
3. **Bundle.** Read the build output. Initial JS per route against the budget. Find what is
   ...
   KB, not an impression.

4. **Runtime performance.** Load the page. Identify the LCP element and confirm nothing
...
5. **Layout stability.** Fonts (metric-matched fallback present?), images (dimensions or
...
6. **Accessibility.** The full pass in `${CLAUDE_PLUGIN_ROOT}/shared/quality/floor.md`:
...
7. **State matrix.** Force each state and look at it: empty, loading, error, offline,
...
8. **Fallbacks.** Every `FALLBACK` the stack record declared, actually exercised: no WebGL,
